"""
Linear Regression model — train, evaluate, predict, save/load.
...
MODEL_PATH = os.path.join(SAVED_DIR, "linear_regression.pkl")
SCALER_PATH = os.path.join(SAVED_DIR, "linear_scaler.pkl")
ENCODER_PATH = os.path.join(SAVED_DIR, "linear_encoder.pkl")


def train(df) -> dict:
    """
    Train a Linear Regression model on the tips dataset.
...
def load_model():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(
            "Linear Regression model not found. Run train() first."
        )
